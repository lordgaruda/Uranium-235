...
int OnInit()
{
    ...
    if(TradingSymbol != "XAUUSD") {
        Print("Warning: This EA is optimized for XAUUSD");
    }
    
    atrHandle = iATR(TradingSymbol, TimeFrame, ATR_Period);
...
void OnTick()
{
    ...
    // Check for buy setup - relaxed conditions
    if(smcData.isBullishCHoCH && smcData.hasFVG && smcData.tradeDirection == 1)
    {
        ExecuteScalpTrade(smcData);
    }
    // Check for sell setup - relaxed conditions
    else if(smcData.isBearishCHoCH && smcData.hasFVG && smcData.tradeDirection == -1)
    {
        ExecuteScalpTrade(smcData);
    }
}
...
void AnalyzeSMCStructure(SMC_Data &data)
{
    ...
    if(data.isBullishCHoCH)
    {
        ...
        if(data.hasFVG)
        {
            ...
            // Allow trade even without order block confirmation
            if(data.hasOrderBlock || true)
...
bool DetectBullishCHoCH()
{
    ...
    if(CopyHigh(TradingSymbol, TimeFrame, 0, CHoCH_LookbackPeriods, high) != CHoCH_LookbackPeriods)
        return false;
    if(CopyLow(TradingSymbol, TimeFrame, 0, CHoCH_LookbackPeriods, low) != CHoCH_LookbackPeriods)
        return false;
    if(CopyClose(TradingSymbol, TimeFrame, 0, CHoCH_LookbackPeriods, close) != CHoCH_LookbackPeriods)
        return false;
...
bool DetectBullishFVG(double &fvgHigh, double &fvgLow)
{
    ...
    if(CopyHigh(TradingSymbol, TimeFrame, 0, 10, high) != 10) return false;
    if(CopyLow(TradingSymbol, TimeFrame, 0, 10, low) != 10) return false;
    if(CopyOpen(TradingSymbol, TimeFrame, 0, 10, open) != 10) return false;
    if(CopyClose(TradingSymbol, TimeFrame, 0, 10, close) != 10) return false;
    ...
    for(int i = 2; i < 8; i++)
    {
        ...
        if(low[i-1] > high[i+1])
        {
            ...
            if(gapSize >= minGapSize)
            {
                ...
                // Check if current price is near or within FVG (more flexible)
                double currentPrice = close[0];
                double buffer = 50 * point; // 5 pip buffer
                return (currentPrice >= (fvgLow - buffer) && currentPrice <= (fvgHigh + buffer));
            }
        }
    }
...
bool DetectBearishOrderBlock(double &orderBlockLevel)
{
    ...
    if(CopyHigh(TradingSymbol, TimeFrame, 0, OrderBlock_LookbackPeriods, high) != OrderBlock_LookbackPeriods) return false;
    if(CopyLow(TradingSymbol, TimeFrame, 0, OrderBlock_LookbackPeriods, low) != OrderBlock_LookbackPeriods) return false;
    if(CopyOpen(TradingSymbol, TimeFrame, 0, OrderBlock_LookbackPeriods, open) != OrderBlock_LookbackPeriods) return false;
    if(CopyClose(TradingSymbol, TimeFrame, 0, OrderBlock_LookbackPeriods, close) != OrderBlock_LookbackPeriods) return false;
...
void ExecuteScalpTrade(SMC_Data &data)
{
    ...
    double currentPrice = isBuy ? SymbolInfoDouble(TradingSymbol, SYMBOL_ASK) : SymbolInfoDouble(TradingSymbol, SYMBOL_BID);
    double point = SymbolInfoDouble(TradingSymbol, SYMBOL_POINT);
    int digits = (int)SymbolInfoInteger(TradingSymbol, SYMBOL_DIGITS);
    
    // Check if price is within FVG zone (more flexible entry)
    bool priceInFVG = (currentPrice >= data.fvgLowerLevel && currentPrice <= data.fvgUpperLevel);
    ...
    if(priceInFVG)
    {
        ...
        double minStopLevel = SymbolInfoInteger(TradingSymbol, SYMBOL_TRADE_STOPS_LEVEL) * point;
        
        if(isBuy)
        {
            if(stopLoss >= currentPrice) stopLoss = currentPrice - MathMax(minStopLevel, 100 * point);
            if(takeProfit <= currentPrice) takeProfit = currentPrice + MathMax(minStopLevel, 200 * point);
        }
        else
        {
            if(stopLoss <= currentPrice) stopLoss = currentPrice + MathMax(minStopLevel, 100 * point);
        ...
        string comment = StringFormat("SMC_%s_FVG", isBuy ? "BUY" : "SELL");
        
        bool tradeSuccess = false;
        if(isBuy)
        {
            tradeSuccess = trade.Buy(calculatedLotSize, TradingSymbol, 0, stopLoss, takeProfit, comment);
        }
        else
        {
            tradeSuccess = trade.Sell(calculatedLotSize, TradingSymbol, 0, stopLoss, takeProfit, comment);
        }
        
        if(tradeSuccess)
        {
            lastTradeTime = TimeCurrent();
            Print("SMC Scalp ", isBuy ? "BUY" : "SELL", " executed at ", currentPrice, 
                  " | SL=", stopLoss, " | TP=", takeProfit, " | Lots=", calculatedLotSize);
        }
        else
        {
            Print("Failed to execute trade. Error: ", trade.ResultRetcode(), 
                  " - ", trade.ResultRetcodeDescription(), 
                  " | Price=", currentPrice, " SL=", stopLoss, " TP=", takeProfit);
        }
    }
}
...
double CalculateTakeProfit(double entryPrice, double stopLoss, double orderBlockLevel, bool isBuy)
{
    ...
    if(orderBlockLevel > 0)
    {
        ...
        if(isBuy && orderBlockLevel > entryPrice)
        {
            ...
            {
                takeProfit = orderBlockLevel - (10 * SymbolInfoDouble(TradingSymbol, SYMBOL_POINT) * 10); // 10 pips before OB
                return takeProfit;
            }
        }
...
void ManageOpenPosition()
{
    ...
    if(UseTrailingStop && positionProfit > 0)
    {
        ...
        if(posType == POSITION_TYPE_BUY)
        {
            ...
            {
                if(trade.PositionModify(TradingSymbol, newStopLoss, takeProfit))
                {
...
{
    return PositionSelect(TradingSymbol);
}
...
double CalculatePositionSize(double entryPrice, double stopLoss)
{
    ...
    double pointValue = SymbolInfoDouble(TradingSymbol, SYMBOL_TRADE_TICK_VALUE);
    double stopDistance = MathAbs(entryPrice - stopLoss);
    
    double calculatedLots = riskAmount / (stopDistance / SymbolInfoDouble(TradingSymbol, SYMBOL_POINT) * pointValue);
